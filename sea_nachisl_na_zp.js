    ...
    // ~~~~~~~~~~~~~~~~~~~~~ Создаются поля ввода ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      var ar_table_head=["ИНН",
                         ...
                         "Подоходный налог (% от оклада)",
                         "Подоходный налог (сумма вычета)",
                         "Фонд обязательного страхования на случай безработицы (%)",
         ...
         div_main1.id="nachisl_na_zp_div_main";
         
    // ~~~~~~~~~~~~~~~~~~~~~ добавляются поля ввода по таблице ~~~~~~~~~~~~~~~~~~

    var st1='<div id="nachisl_na_zp_inline">'+
            '  <div id="nachisl_na_zp_block">'+
                  '<div id="nachisl_na_zp_inline">'+
                  '  <div id="record_id"></div>';
                   
                     st1+='</div>';
                  
                     st1+=' <input id="nnzp_set_all" type="checkbox" data-action="nnzp_set_all">Значение установить всем</input>';
       ...
       if ((selected_TD!=null) && (selected_TR!=null)) {
        ...
        for (var i=0; i<selected_TR.cells.length; i++) {
           ...
           if (selected_TR.cells[i]==selected_TD) {
            ...
            setTimeout(nachisl_na_zp_input_show, 1);
       }
     }

    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

     function nachisl_na_zp_input_show() {
     ...
     function insert_row_to_table() {
      ...
      for (var j=0; j<fbody1.rows.length; j++) {
           ...
           s1+="INSERT INTO sea_nachisl_na_zp (year_mon, inn, pn_sum, pn_proc, pn_sum_vycheta, fossb, fosvn, pensia_fond, alimenty, avans) "+
     ...
     function update_row_to_table() {
      ...
      for (var j=0; j<fbody1.rows.length; j++) {
               ...
               s1+="UPDATE sea_nachisl_na_zp SET year_mon="+ym1+", inn="+fin1.innerHTML;

               fin1=fbody1.rows[j].cells[2];
               s1+=", pn_sum="+sea_get_number(fin1.innerHTML);

               fin1=fbody1.rows[j].cells[3];
               s1+=", pn_proc="+sea_get_number(fin1.innerHTML);
               
               fin1=fbody1.rows[j].cells[4];
               s1+=", pn_sum_vycheta="+sea_get_number(fin1.innerHTML);

               fin1=fbody1.rows[j].cells[5];
               s1+=", fossb="+sea_get_number(fin1.innerHTML);

               fin1=fbody1.rows[j].cells[6];
               s1+=", fosvn="+sea_get_number(fin1.innerHTML);

               fin1=fbody1.rows[j].cells[7];
               s1+=", pensia_fond="+sea_get_number(fin1.innerHTML);

               fin1=fbody1.rows[j].cells[8];
               s1+=", alimenty="+sea_get_number(fin1.innerHTML);
     ...
     function add_table_row() {
         ...
         mysql_post("http://192.168.0.151:5050/?"+s1, this.callback_post_nachisl_na_zp_update);

     }

     function callback_post_nachisl_na_zp_update() {
	   ...
	   if (is1) {
	     ...
	     mysql_post("http://192.168.0.151:5050/?"+s1, this.callback_post_nachisl_na_zp_insert);
	   }
     }
     function callback_post_nachisl_na_zp_insert() {
     ...
     function select_pomowi() {
         ...
         var s1="SELECT a.inn as inn, CONCAT(CONCAT(a.family, ' '), CONCAT(CONCAT(a.name, ' '), a.father)) AS `fio`,"+
                "(SELECT b.pn_sum          FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as pn_sum,"+
                "(SELECT b.pn_proc         FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as pn_proc,"+
                "(SELECT b.pn_sum_vycheta  FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as pn_sum_vycheta,"+
		"(SELECT b.fossb           FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as fossb,"+
		"(SELECT b.fosvn           FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as fosvn, "+
		"(SELECT b.pensia_fond     FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as pensia_fond, "+
		"(SELECT b.alimenty        FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as alimenty, "+
		"(SELECT b.avans           FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as avans, "+
		"(SELECT b.ID              FROM `sea_nachisl_na_zp` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as ID "+
	...
	for (var i=0; i<(r1.length-1); i++) {
	    ...
	    var r2=r1[i].split('\r');
          ...
          for (var j=0; j<ar_table_head.length; j++) {
            ...
            if (r2[j+1]!='null') {
                  cell0.innerHTML=(j>1 ? Number(r2[j+1]).toFixed(2) : r2[j+1]);
            }
          }
	}
