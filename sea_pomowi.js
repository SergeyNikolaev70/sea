    ...
    // ~~~~~~~~~~~~~~~~~~~~~ Создаются поля ввода ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      var ar_table_head=["ИНН",
         ...
         div_main1.id="pomowi_div_main";
         
    // ~~~~~~~~~~~~~~~~~~~~~ добавляются поля ввода по таблице ~~~~~~~~~~~~~~~~~~

    var st1='<div id="pomowi_inline">'+
            '  <div id="pomowi_block">'+
                  '<div id="pomowi_inline">'+
     ...
     t_data.ondblclick=function(event) {
       ...
       while (trgt!=t1) {
           ...
           if (trgt.tagName=='TR') {
                 ...
                 setTimeout(pomowi_input_show, 1);
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     function insert_row_to_table() {
      ...
      for (var j=0; j<fbody1.rows.length; j++) {
           ...
           s1+="INSERT INTO sea_pomowi (year_mon, inn, mat_pomow, netrudosposobnost, summa) "+
     ...
     function update_row_to_table() {
      ...
      for (var j=0; j<fbody1.rows.length; j++) {
               ...
               s1+="UPDATE sea_pomowi SET year_mon="+ym1+", inn="+fin1.innerHTML;

               fin1=fbody1.rows[j].cells[2];
               s1+=", mat_pomow="+sea_get_number(fin1.innerHTML);

               fin1=fbody1.rows[j].cells[3];
               s1+=", netrudosposobnost="+sea_get_number(fin1.innerHTML);
     ...
     function add_table_row() {
         ...
         mysql_post("http://192.168.0.151:5050/?"+s1, this.callback_post_pomowi_update);

     }

     function callback_post_pomowi_update() {
	   ...
	   if (is1) {
	     ...
	     mysql_post("http://192.168.0.151:5050/?"+s1, this.callback_post_pomowi_insert);
	   }
     }
     function callback_post_pomowi_insert() {
     ...
     function select_pomowi() {
         ...
         var s1="SELECT a.inn as inn, CONCAT(CONCAT(a.family, ' '), CONCAT(CONCAT(a.name, ' '), a.father)) AS `fio`,"+
                "(SELECT b.mat_pomow         FROM `sea_pomowi` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as mat_pomow,"+
                "(SELECT b.netrudosposobnost FROM `sea_pomowi` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as netrudosposobnost,"+
                "(SELECT b.summa             FROM `sea_pomowi` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as summa,"+
		"(SELECT b.ID                FROM `sea_pomowi` AS b WHERE (b.inn=a.inn) AND (b.year_mon='"+tabel_year+"-"+tabel_month+"')) as ID "+                
